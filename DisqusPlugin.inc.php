...
class DisqusPlugin extends GenericPlugin {
	...
	function register($category, $path, $mainContextId = null) {
		...
		if ($success && $this->getEnabled(($mainContextId))) {
			...
			HookRegistry::register('Templates::Preprint::Footer::PageFooter', array($this, 'addDisqus'));
	...
	*/
	...
	function addDisqus($hookName, $params) {
		...
		else if ($templateMgr->get_template_vars('preprint')){
			$submission = $templateMgr->get_template_vars('preprint');
			$submissionId = $submission->getBestId();
			$submissionUrl = $request->url($context->getPath(), 'preprint', 'view', $submissionId);
		}
		else if ($templateMgr->get_template_vars('article')){
